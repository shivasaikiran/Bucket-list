...
function ProductCard() {
    ...
    const { mode,product, searchkey,setSearchkey,filterType,setFilterType,filterPrice,setFilterPrice
    } = context
                        ...
                        .filter((obj) => obj.price.includes(filterPrice)).slice(0,8).map((item, index) => {
                            ...
                            return (
                                <div onClick={()=> window.location.href = `/productinfo/${id}`}  className="p-4 md:w-1/4 drop-shadow-lg " >
                                    <div key={index} className="h-full overflow-hidden transition-shadow duration-300 ease-in-out border-2 border-gray-200 hover:shadow-gray-100 hover:shadow-2xl border-opacity-60 rounded-2xl" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                        ...
                                        <div className="p-5 border-t-2">
                                            ...
                                            <div className="flex justify-center ">
                                                <button onClick={() => addCart(item)} type="button" className="w-full py-2 text-sm font-medium text-white bg-pink-600 rounded-lg focus:outline-none hover:bg-pink-700 focus:ring-4 focus:ring-purple-300">Add To Cart</button>
