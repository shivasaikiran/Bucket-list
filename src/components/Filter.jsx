...
function Filter() {
    ...
    const { mode,searchkey,setSearchkey,filterType,setFilterType,filterPrice,setFilterPrice,product
    } = context
           ...
           <div className='container px-4 mx-auto mt-5 '>
                <div className="p-5 bg-gray-100 border border-gray-200 rounded-lg drop-shadow-xl"
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4 md:grid-cols-3 xl:grid-cols-4">
                        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full px-4 py-3 text-sm border-transparent rounded-md bg-gray-50 outline-0 focus:border-gray-500 focus:bg-white focus:ring-0" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                        ...
                        <select value={filterPrice} onChange={(e) => setFilterPrice(e.target.value)} className="w-full px-4 py-3 text-sm border-transparent rounded-md bg-gray-50 outline-0 focus:border-gray-500 focus:bg-white focus:ring-0" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                            {product.map((item, index) => {
                                return (
